...
struct OperationalStandardStatusRouteTests {
    ...
    @Test("Only the two ingestion routes remain in the composition root")
    func remainingRegistrationsArePinned() throws {
        ...
        // Since F-G10 the composition root registers nothing but ingestion, which is
        // deliberately last in the extraction order.
        #expect(remaining == [
            "POST /api/v1/audio",
            "POST /api/v1/sync"
